...
//
static int      logdata_open(LOGDATA* logdata, const char* path, const char* name, LOGLEVEL level);
static int      logdata_close(LOGDATA* logdata);
static int      logdata_write(LOGDATA* logdata, LOGLEVEL level, const char* fmt, va_list args);
static char*    get_timestr(char* buff, const int buff_size, struct tm* t);
static bool     is_next_day(LOGDATA* logdata, struct tm* t);

// global variables defined here
// 
LOGDATA g_logdata_norm={false, stderr, false, PTHREAD_MUTEX_INITIALIZER, PY_LOG_NOTSET, 0,0,0, "", ""};
LOGDATA g_logdata_warn={false, stderr, false, PTHREAD_MUTEX_INITIALIZER, PY_LOG_WARN, 0,0,0, "", ""};
 ...
 */
int py_log_open(const char* path, const char* name, LOGLEVEL level)
{
	char wname[256];
	//char timestr[30];
...
int py_log_open(const char* path, const char* name, const char* level_str)
{
	...
	return py_log_open(path, name, level);
 ...
 * ret  : 0, succeed; else error
 */
int py_log_write(LOGLEVEL level, const char* fmt, ...)
{
	int     ret  = 0;
	va_list args;
 ...
 *      : -1, error
 */
int logdata_open(LOGDATA* logdata, const char* path, const char* name, LOGLEVEL level)
{
	...
	logdata->isopen         = true;
	if(logdata->is_lock_init==false){
		pthread_mutex_init(&(logdata->lock), NULL);
		logdata->is_lock_init=true;
	}
 ...
 *      : -1, error
 */
int logdata_write(LOGDATA* logdata, LOGLEVEL level, const char* fmt, va_list args)
{
	int  ret        = 0;
	int  len        = 0;
	int  bufflen    = 0;
 ...
 * ret  :
 */
bool is_next_day(LOGDATA* logdata, struct tm* t)
{
	
	if(logdata->isopen == false){
		return false;
	}
